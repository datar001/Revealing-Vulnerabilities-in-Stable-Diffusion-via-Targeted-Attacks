...
class ModifiedStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def _new_encode_prompt(self, prompt, device, num_images_per_prompt, do_classifier_free_guidance,
                           negative_prompt, prompt_ids=None, prompt_embeddings=None, delete_id=None):
        r"""
        ...
        if delete_id is not None:
            mask = torch.ones(77).bool().to(text_embeddings.device)
            id_mask = torch.LongTensor(delete_id).to(text_embeddings.device)
            mask[id_mask] = False
    ...
    def forward_intermediate(
        ...
        # 3. Encode input prompt
        text_embeddings = self._new_encode_prompt(
            prompt, device, num_images_per_prompt, do_classifier_free_guidance, negative_prompt, prompt_ids,
            prompt_embeddings, delete_id
        )
        ...
        # 8. Post-processing
        images = []
        for latents in intermediate_latents:
            image = self.decode_latents(latents)
            images.append(image)
        ...
        return StableDiffusionPipelineOutput(images=pil_images, nsfw_content_detected=None)

    @torch.no_grad()
    def gen_images_by_selected_token_id(
            self,
            prompt: Union[str, List[str]],
            delete_id: Optional[List]=None,
            exchange_latent_step: Optional[int]=None,
            height: Optional[int] = None,
            width: Optional[int] = None,
            num_inference_steps: int = 50,
            guidance_scale: float = 7.5,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    ...
                    if callback is not None and i % callback_steps == 0:
                        callback(i, t, latents)
                if exchange_latent_step is not None:
                    import pdb
                    pdb.set_trace()
                    if i == exchange_latent_step:
                        pass
